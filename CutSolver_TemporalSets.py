...
class CutSolverT(object):
    ...
    def __init__(self):
        ...
        self._rank_list_tri = []
        self._idx_list_tri = []
        # List storing information of selected cuts of previous rounds
        self.all_chosen_cuts_nfeas, self.all_chosen_cuts_idxlist, self.duplicates_list, self.duplicates_feas,\
              self.n_replacements, self.all_chosen_cuts, self.n_cuts_added =[], [], [], [], [], [], []
        self.n_common_triplets_encountered, self.n_common_triplets_replaced, self.sel_size_round =0, 0, 0

    def cut_select_algo(self, filename, dim, sel_size, strat=2, nb_rounds_cuts=20, term_on=False,
                        triangle_on=False, ch_ext=False, strong_only=False, sol=0, all_comp=False):
        ...
        if nb_subprobs >= CutSolverT._THRES_MAX_SUBS or nb_rounds_cuts == 0:
        ...
        sel_size = min(int(np.floor(sel_size * nb_subprobs)) if sel_size < 1 else min(sel_size, nb_subprobs),
                       CutSolverT._SDP_CUTS_PER_ROUND_MAX)
        ...
        ######################################################
        self.all_chosen_cuts_idxlist, self.duplicates_list, self.n_replacements, self.n_cuts_added =[], [], [], []
        ...
        for cut_round in range(1, nb_rounds_cuts + 1):
            ...
            if strat == 0:
                ...
                if  strat != 4:
                    ...
                    rank_list = self._sel_unique_eigcut_with_temporal_sets( strat, rank_list, cut_round, sel_size)
                else:    # combined selection
                ...
                nb_sdp_cuts = self._gen_eigcuts_selected(strat, sel_size, rank_list, cut_round,
        ...
        # End of cut rounds - return info 
        ######################################################
        
        # print('Total replaced cuts: ',(self.n_replacements),'\nTotal added cuts: ',(self.n_cuts_added) )
        return ([-obj for obj in curr_obj_vals], timer() - time_begin, round_times,
                sep_times, nbs_sdp_cuts, nbs_tri_cuts, vars_values, agg_list, self.n_cuts_added, self.n_replacements)    
    ...
    def _get_sdp_vertex_cover(self, dim, ch_ext=0): 
        ...
        if len(agg_list) >= CutSolverT._THRES_MAX_SUBS:
            return len(agg_list)          
    ...
    def _sel_eigcut_by_ordering_on_measure(self, strat, vars_values, cut_round, sel_size=0):
        ...
        if opt_sel or comb_sel or exact_sel:
            ...
            for agg_idx, (set_inds, Xarr_inds, Q_slice, max_elem) in enumerate(agg_list):
                ...
                # Optimality selection via exact SDP solution (Mosek)
                    ...
                    con1.add(x_pos)
                    con2.add(x_pos)
                rank_list[agg_idx] = (agg_idx, obj_improve,set_inds, curr_pt, X_slice)
            ...
            if comb_sel:
                ...
                violated_cuts = 0
                for ix, (agg_idx,set_inds, obj_improve, curr_pt, X_slice) in enumerate(rank_list):  
                    if obj_improve > CutSolverT._THRES_MIN_OPT and strong_violated_cuts < sel_size:      
                        # Check smallest eigenvalue for violated cuts
                        eigval = get_eigendecomp(len(curr_pt), curr_pt, X_slice, False)[0]
                        if eigval < CutSolverT._THRES_NEG_EIGVAL:    # violated strong cut
                            rank_list[ix] = (agg_idx,set_inds, obj_improve + CutSolverT._BIG_M, curr_pt, X_slice)
                    ...
                    # if not enough strong violated cuts can be found, employ also selection by feasibility
                        ...
                        eigval = get_eigendecomp(len(curr_pt), curr_pt, X_slice, False)[0]
                        if eigval < CutSolverT._THRES_NEG_EIGVAL:    # violated cut
                            rank_list[ix] = (agg_idx,set_inds, -eigval, curr_pt, X_slice)
                            violated_cuts += 1
                    else:
                        break
                
                rank_list.sort(key=itemgetter(2), reverse=True) # Sort again
        ...
        # Feasibility (-only) selection
            ...
            for agg_idx, (set_inds, Xarr_inds, _, _) in enumerate(agg_list):
                ...
                if eigval < CutSolverT._THRES_NEG_EIGVAL:
                    rank_list[agg_idx] = (set_inds, -eigval, Xarr_inds, dim_act, curr_pt)
                    nb_violated += 1
                else:
    ...
    def _gen_eigcuts_selected(self, strat, sel_size, rank_list, cut_round, strong_only=False, vars_values=None):
        ...
        """
        nb_subprobs, my_prob, nb_lifted, duplicates_list = len(self._agg_list), self._my_prob, self._nb_lifted, self.duplicates_list
        nb_sdp_cuts, coeffs_sdp, rhs_sdp, senses_sdp, names_triplets, duplicates_feas = 0, [], [], [], [], []
        opt_sel, feas_sel, rand_sel = (strat in [2, 3, 4, -1]), (strat == 1), (strat == 5)
        sel_size=self.sel_size_round
        ...
        ix = 0
        if duplicates_list:
            duplicates_idx_list=[str(t[0]) for t in duplicates_list]
            my_prob.linear_constraints.delete(duplicates_idx_list)
            rank_list.extend(duplicates_list) 
            sel_size=sel_size+len(duplicates_list)        
        while nb_sdp_cuts < sel_size and ix < sel_size:
            ...
            if not feas_sel:
                if opt_sel:
                    (idx, diff,set_inds, curr_pt, X_slice) = rank_list[ix]
                ...
                (set_inds, _, Xarr_inds, dim_act,_) = rank_list[ix]
                curr_pt = itemgetter(*set_inds)(x_vals)
                X_slice = itemgetter(*Xarr_inds)(X_vals)
            ...
            #################
            if eigvals[0] < CutSolverT._THRES_NEG_EIGVAL:
                evect = np.where(abs(evect) <= -CutSolverT._THRES_NEG_EIGVAL, 0, evect)
                ...
                senses_sdp.append("G")
                names_triplets.append(str(set_inds))
                nb_sdp_cuts += 1
            ix += 1
        # Add all SDP eigen-cuts at once to problem instance
        my_prob.linear_constraints.add( lin_expr=coeffs_sdp, rhs=rhs_sdp, senses=senses_sdp, names=names_triplets)
        self.duplicates_feas.extend(duplicates_feas)
        return nb_sdp_cuts
    ...
    def __gen_dense_eigcuts(self, vars_values=None):
        ...
        ix = 0
        while ix < dim_act:
            if eigvals[ix] < CutSolverT._THRES_NEG_EIGVAL:
                evect = evecs.T[ix]
                evect_arr = [evect[idx1] * evect[idx2] * 2 if idx1 != idx2 else evect[idx1] * evect[idx2]
                             for idx1 in range(dim_act + 1) for idx2 in range(max(idx1, 1), dim_act + 1)]
                coeffs_sdp.append(cplex.SparsePair(
        ...
        for i1 in range(nb_vars):
            for i2 in range(i1 + 1, nb_vars):
                for i3 in range(i2 + 1, nb_vars):
                    ...
                    if density >= CutSolverT._THRES_TRI_DENSE:
                        set_inds = [i1, i2, i3]
    ...
    def __separate_and_add_triangle(self, sel_size, vars_values):
        ...
        rank_list_tri_viol = [el for el in rank_list_tri if el[3] >= CutSolverT._THRES_TRI_VIOL]
        ...
        nb_tri_cuts = max(min(CutSolverT._TRI_CUTS_PER_ROUND_MIN, int(np.floor(sel_size * len(rank_list_tri_viol)))),
                          min(CutSolverT._TRI_CUTS_PER_ROUND_MAX, len(rank_list_tri_viol)))
    ...
    def _sel_unique_eigcut_with_temporal_sets(self, strat, rank_list, cut_round, sel_size):
        ''' Temporal method: 
        (i) Choose a diverse set of cuts using the affinity metric
        (ii) Choose unique sets of triplets 
        (iii) Replace co-linear cuts   '''

        assert (strat ==1) # has only been applied in combination with the feasibility measure
        all_chosen_cuts_idxlist=self.all_chosen_cuts_idxlist
        all_chosen_cuts=self.all_chosen_cuts
        keep_list, keep_list_idx, duplicates_list=[], [], []
        rank_list_idx = [t[0] for t in rank_list] 
        counter=0
        if sel_size> 1:
            self.sel_size_round=sel_size
        else:
            self.sel_size_round=sel_size*len(rank_list)

        for i, triplet in enumerate(rank_list_idx):
            if triplet not in all_chosen_cuts_idxlist:
                valid = True
                for element in keep_list_idx:
                    common_elements = len(set(triplet).intersection(element))
                    if common_elements == 2 :                       
                        valid = False
                        break
                if valid:
                    keep_list.append(rank_list[i])
                    keep_list_idx.append(triplet)
                    all_chosen_cuts.append(rank_list[i])

                    if len(keep_list)>= self.sel_size_round:                    
                        break
            else:
                counter+=1 
                self.n_common_triplets_encountered+=1
                self.n_common_triplets_replaced+=1 
                cut_position=all_chosen_cuts_idxlist.index(triplet)
                 
                duplicates_list.append(rank_list[i])
                all_chosen_cuts[cut_position]=rank_list[i]
         
        self.all_chosen_cuts_idxlist.extend(keep_list_idx)
        self.duplicates_list= duplicates_list
        self.n_replacements.append(str(len(duplicates_list))+'/'+str(counter))
        self.n_cuts_added.append(len(keep_list))
        return keep_list
